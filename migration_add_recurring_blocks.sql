...
BEGIN
    ...
    current_date := parent_block.block_date;
    ...
    CASE parent_block.recurrence_type
        WHEN 'daily' THEN
            ...
            WHILE current_date <= end_date LOOP
                INSERT INTO schedule_blocks (
                    ...
                    parent_block.barber_id, current_date, parent_block.start_time, 
        ...
        WHEN 'weekly' THEN
            ...
            FROM jsonb_array_elements_text(parent_block.recurrence_pattern->'days');
            
            current_date := current_date + 1;
            
            WHILE current_date <= end_date LOOP
                -- Verificar se o dia da semana atual está na lista
                IF EXTRACT(DOW FROM current_date)::INTEGER = ANY(days_of_week) THEN
                    INSERT INTO schedule_blocks (
                        barber_id, block_date, start_time, end_time, reason,
                        is_recurring, recurrence_type, recurrence_pattern, 
                        recurrence_end_date, parent_block_id, created_by
        ...
        WHEN 'monthly' THEN
            ...
            current_date := (DATE_TRUNC('month', current_date) + INTERVAL '1 month' + (day_of_month - 1 || ' days')::INTERVAL)::DATE;
            
            WHILE current_date <= end_date LOOP
                -- Verificar se o dia existe no mês (evitar 31 de fevereiro, etc.)
                IF EXTRACT(DAY FROM current_date) = day_of_month THEN
                    INSERT INTO schedule_blocks (
                        barber_id, block_date, start_time, end_time, reason,
                        is_recurring, recurrence_type, recurrence_pattern, 
                        recurrence_end_date, parent_block_id, created_by
